...
set MODELS_FOLDER=%DOCKER_FOLDER%\comfy_ui\models
set CUSTOM_NODES_FOLDER=%DOCKER_FOLDER%\comfy_ui\custom_nodes
...
if not exist "%MODELS_FOLDER%\loras" mkdir "%MODELS_FOLDER%\loras"
if not exist "%MODELS_FOLDER%\clip" mkdir "%MODELS_FOLDER%\clip"
if not exist ".\workflow" mkdir ".\workflow"

set /p answer=Do you want to get the ByteDance/SDXL-Lightning model (y/N)? 
if /i "%answer%" EQU "Y" (
  set "FILE_PATH=%MODELS_FOLDER%\loras\sdxl_lightning_4step.safetensors"
  IF NOT EXIST "!FILE_PATH!" (
    echo Downloading ByteDance/SDXL-Lightning model to !FILE_PATH!...
    curl -L -o "!FILE_PATH!" https://huggingface.co/ByteDance/SDXL-Lightning/resolve/main/sdxl_lightning_4step.safetensors?download=true
  ) ELSE (
    echo !FILE_PATH! already exists, skipping download.
  )
    REM Download worflow
    curl -L -o .\workflow\sdxl_lightning_workflow_full.json https://huggingface.co/ByteDance/SDXL-Lightning/resolve/main/comfyui/sdxl_lightning_workflow_full.json
)

set /p answer=Do you want to get the FLUX.1 [dev] model (y/N)? 
if /i "%answer%" EQU "Y" (
  set "FILE_PATH=%MODELS_FOLDER%\checkpoints\flux1-dev-fp8.safetensors"
  IF NOT EXIST "!FILE_PATH!" (
    echo Downloading FLUX.1 [dev] model to !FILE_PATH!...
    curl -L -o "!FILE_PATH!" https://huggingface.co/Kijai/flux-fp8/resolve/main/flux1-dev-fp8.safetensors?download=true
    curl -L -o "%MODELS_FOLDER%\clip\clip_l.safetensors" https://huggingface.co/comfyanonymous/flux_text_encoders/resolve/main/clip_l.safetensors?download=true
    curl -L -o "%MODELS_FOLDER%\clip\t5xxl_fp8_e4m3fn.safetensors" https://huggingface.co/comfyanonymous/flux_text_encoders/resolve/main/t5xxl_fp8_e4m3fn.safetensors?download=true
    curl -L -o "%MODELS_FOLDER%\vae\ae.safetensors" https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/ae.safetensors?download=true
  ) ELSE (
    echo !FILE_PATH! already exists, skipping download.
  )
)
