...
function configure_service_backup() {
    ...
    # 1. Define the service name, absolute path and script targets first
    ...
    # Search for backup.sh in the git repo under this service's folder structure
    local custom_git_script=""
    if [ -d "$REPO_ROOT/cloud-services/$service_name" ]; then
        custom_git_script="$REPO_ROOT/cloud-services/$service_name/backup.sh"
    elif [ -d "$REPO_ROOT/infrastructure/$service_name" ]; then
        custom_git_script="$REPO_ROOT/infrastructure/$service_name/backup.sh"
    elif [ -d "$REPO_ROOT/ai/$service_name" ]; then
        custom_git_script="$REPO_ROOT/ai/$service_name/backup.sh"
    fi

    # If there is a custom script in Git that is NOT a generated template, use it!
    if [ -f "$custom_git_script" ] && ! grep -q "Generated by backup-utils.sh" "$custom_git_script"; then
        print_info "Found a custom source code script for $service_name in Git. Copying it to the installation..."
        cp "$custom_git_script" "$backup_script_path"
        chmod +x "$backup_script_path"
    # Alternatively: If the file in the installed folder is already modified by the user directly on the server
    elif [ -f "$service_dir/backup.sh" ] && ! grep -q "Generated by backup-utils.sh" "$service_dir/backup.sh"; then
        print_info "Keeping locally modified backup script for $service_name."
        chmod +x "$backup_script_path"
    else
        # If no custom script is found anywhere, generate from template as usual
        if [ ! -f "$template" ]; then
