...
function configure_service_backup() {
    ...
    # If there already is a specific backup script in the source code, don't overwrite it!
    if [ -f "$service_dir/backup.sh" ] && ! grep -q "Generated by backup-utils.sh" "$service_dir/backup.sh"; then
        print_info "Hittade ett anpassat backup-skript för $service_name. Använder det istället för mallen."
        chmod +x "$backup_script_path"
    else
        # Om inget anpassat skript finns, generera från mall som vanligt
        if [ ! -f "$template" ]; then
